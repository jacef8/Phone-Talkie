...
function connectWS() {
  ...
  ws.onopen = () => {
    ...
    if (savedName && currentRoom && window._wasInRoom) {
      myName = savedName;
      // Clean up all stale peer connections before rejoining
      peers.forEach((_, id) => closePeer(id));
      currentRoom = null; // reset so renderRoom works fresh
...
async function startTx(e) {
  ...
  // Add mic track to all peer connections if not already added
  peers.forEach((pc) => {
    const senders = pc.getSenders();
