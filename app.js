...
function connectWS() {
  ...
  ws.onopen = () => {
    ...
    const wasConnected = window._wasConnected || false;
    window._wasConnected = true;
    reconnectDelay = 2000;
    console.log('WS connected');
    updateConnectionStatus(true);
    // Only auto-rejoin if this is a reconnect (not first load)
    // and we were actively in a room when connection dropped
    const lastRoom = loadLastRoom();
    const savedName = localStorage.getItem('breaker-name');
    if (wasConnected && lastRoom && savedName && !currentRoom) {
      myName = savedName;
      getMic().then(() => {
        send({ type: 'join-room', code: lastRoom.code, peerName: savedName });
      }).catch(console.error);
    }
...
async function handleSignal(msg) {
  switch (msg.type) {
    ...
    case 'error':
      // Clear the saved last room if it no longer exists
      if (msg.message === 'Room not found') {
        localStorage.removeItem('breaker-last-room');
        showToast('Room no longer active — create or join a new one');
      } else {
...
function leaveRoom() {
  ...
  currentRoom = null;
  localStorage.removeItem('breaker-last-room'); // clear so we don't auto-rejoin
  releaseMic();
  setWave(null);
...
document.addEventListener('visibilitychange', async () => {
  if (document.hidden) {
    // Page hidden — release mic so other apps can use it
    if (localStream) {
      localStream.getTracks().forEach(t => t.stop());
      localStream = null;
      console.log('Mic released — page hidden');
    }
  } else {
    // Page visible again — if in a room, re-acquire mic
    if (currentRoom && !localStream) {
      try {
        ...
        console.log('Mic re-acquired — page visible');
      } catch(e) {
        console.error('Could not re-acquire mic:', e);
      }
    }
  }
});
