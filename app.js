...
// Get a silent track using a real mic but immediately disabled
// This avoids AudioContext requiring a user gesture on mobile
let _silentStream = null;
async function getSilentTrack() {
  if (_silentStream) return _silentStream.getAudioTracks()[0].clone();
  try {
    _silentStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
    _silentStream.getAudioTracks().forEach(t => { t.enabled = false; });
    return _silentStream.getAudioTracks()[0].clone();
  } catch(e) {
    // Fallback to AudioContext if mic denied
    const ctx = new (window.AudioContext || window.webkitAudioContext)();
    const dst = ctx.createMediaStreamDestination();
    window._silentCtx = ctx;
...
async function createOffer(peerId) {
  ...
  // Add a silent track so audio is negotiated from the start
  const silentTrack = await getSilentTrack();
...
async function startTx(e) {
  ...
  // Replace silent track with real mic track - no renegotiation needed
  const replacePromises = [];
  peers.forEach((pc) => {
    const audioSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
    if (audioSender) {
      replacePromises.push(audioSender.replaceTrack(audioTrack).catch(console.error));
    }
  });
...
async function stopTx() {
  ...
  transmitting = false;
  // Replace mic with silent track, then stop mic
  const silentTrack = await getSilentTrack();
  peers.forEach((pc) => {
    const audioSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
    if (audioSender) audioSender.replaceTrack(silentTrack).catch(console.error);
  });
  if (localStream) {
